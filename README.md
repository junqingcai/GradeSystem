...
本项目是一个使用 C 语言编写的“学生成绩管理系统”课程设计项目。程序采用菜单驱动方式运行，使用单向链表在内存中保存学生成绩信息，并通过 `data.txt` 文件实现数据的读取和保存。

项目不使用数据库，不依赖第三方库，适合大一 C 语言课程设计、实验报告和课堂答辩展示。
...
| 成员 | 负责文件 | 主要任务 |
|---|---|---|
| A | `main.c` | 程序入口、菜单显示、用户选择、整体流程控制 |
...
- 自动编译 `main.c`、`student.c`、`file.c`、`analysis.c`
- 自动把 `data.txt` 复制到 CMake 构建目录
- Windows 下尽量处理中文显示乱码问题

## 八、命令行编译运行

如果使用 MinGW/GCC，可以在项目目录下执行：

```bash
gcc main.c student.c file.c analysis.c -o GradeSystem.exe -Wall -Wextra -std=c99 -finput-charset=UTF-8 -fexec-charset=GBK
```
...
如果在 Linux/macOS 下运行，可使用：

```bash
gcc main.c student.c file.c analysis.c -o GradeSystem -Wall -Wextra -std=c99
./GradeSystem
```

## 九、中文乱码处理

本项目源码使用 UTF-8 保存。为了适配 Windows 中文环境，程序在 Windows 下会自动设置控制台代码页为 `936`，并且 CMake 中设置了对应编译选项。

如果在 CLion 中仍然出现中文乱码，请尝试：

1. 删除项目目录下的 `cmake-build-debug` 文件夹
