...
// 读取一个不含空格的字词，data.txt 中的名称也遵循此规则
...
// 根据学号删除节点。如果被删除的节点不为NULL，则将删除的数据返回给调用者
...
void deleteFailedStudents(Node *head) {
    ...
    prev = head;
    // 当删除一个节点时，不向前移动 prev，因为 prev->next 已经发生了变化
...
void insertStudent(Node *head) {
    ...
    // 复用 inputStudent 函数，使插入的记录遵循相同的验证规则
    stu = inputStudent(head, 1);
