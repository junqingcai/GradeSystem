...
static void initConsoleEncoding(void) {
    /*
     * 需要修复 CLion 所使用的 Windows 终端上的中文乱码问题
     * 源文件以 UTF-8 编码保存。在 Windows 上，CMake 会将中文字符串字面量编译为 GBK 编码
     * 并且控制台会被切换到代码页 936。在 Linux/macOS 上，则继续正常使用 UTF-8 编码
     * 可将data以记事本格式打开后以ANSI形式另存并覆盖原文件
     */
#ifdef _WIN32
...
int main(void) {
#ifdef _WIN32
    SetConsoleOutputCP(936);   // 设置为 GBK 编码
#endif
